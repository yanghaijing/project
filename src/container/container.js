...
import Head from '../componment/head';
import LeftSelect from'../componment/left_select';
import Mask from '../componment/mask';
import MaskAdd from '../componment/mask-add'
...
class Container extends Component {
    constructor(props) {
        ...
        //this.changeSalaryIndex=this.changeSalaryIndex.bind(this);
    ...
    render() {
        ...
        if(this.state.set===1){
            ...
            children=React.cloneElement(this.props.children,{
                showMask:this.showMask,
                student: this.state.data.students
            })
        }
        return (
            <div ref='scroll' style={{width:'100%',height:'100%',overflow:'auto'}} onScroll={this.scroll}>
                {
                    this.selectMask()
                }
                <Head grade={this.state.data.grade} period={this.state.data.period}
                    course={this.state.data.courseList} teacherList={this.state.data.teacherList}
                      Headclick={this.Headclick} src={this.state.set===1?set:back}/>
                {
                    children
                }
                <LeftSelect students={this.state.data.students} pictureClick={this.pictureClick}
                    changeIndex={this.changeIndex} clickedStudent={this.state.index}
                    />
            </div>
        );
    }
    ...
    Headclick(){
        ...
        }else if(this.state.set===1){
            hashHistory.push("/teacher/content1")
        }else if(this.state.set===2){
            hashHistory.push("/teacher/index")
        }
    }
    scroll() {
        ...
        if(this.refs.scroll.clientHeight+this.refs.scroll.scrollTop==this.refs.scroll.scrollHeight){
            this.setState({
                addDate:this.state.addDate+1
            });
        }
    }
    selectMask(){
        if(this.state.set===1){
            return  <Mask showMask={this.state.showMask}
                          cancelMask={this.cancelMask}
                          changeScore={this.changeScore}
                          name="奖励加薪项目" name1="惩罚减薪项目"
                />
        }else if(this.state.set===3){
            return  <MaskAdd showMask={this.state.showMask}
                             cancelMask={this.cancelMask}
    ...
    componentWillReceiveProps(props){
        ...
        }else if(props.location.pathname=='/teacher'||props.location.pathname=='/teacher/look'){
            this.setState({
                set:2
            })
        }else if(props.location.pathname=='/teacher'||props.location.pathname=='/teacher/content1'){
            this.setState({
                set:3
            })
        }
